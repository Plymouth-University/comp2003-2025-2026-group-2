...
<div class="flex min-h-full w-full items-center justify-center p-6" style="background-color: var(--bg-secondary);">
	<div class="w-full max-w-md rounded-lg border-2 p-8 shadow-lg" style="background-color: var(--bg-primary); border-color: var(--border-primary);">
		<div class="mb-6 text-center">
			<div class="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full" style="background-color: #fee2e2;">
			...
			<h1 class="mb-2 text-2xl font-bold" style="color: var(--text-primary);">Confirm Branch Deletion</h1>
			<p class="text-sm" style="color: var(--text-secondary);">This is a permanent action that cannot be undone.</p>
		</div>

		{#if isLoading}
			<div class="py-8 text-center">
				<div class="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-b-2" style="border-color: var(--text-primary);"></div>
				<p style="color: var(--text-secondary);">Loading...</p>
			</div>
		{:else if success}
			<div class="mb-6 rounded-lg p-4 text-center" style="background-color: #dcfce7; color: #166534;">
		...
		{:else if error && !token}
			<div class="mb-6 rounded-lg p-4 text-center" style="background-color: #fee2e2; color: #dc2626;">
		...
		{:else}
			<div class="mb-6 rounded-lg border-l-4 p-4" style="background-color: #fef3c7; border-color: #f59e0b;">
				<p class="mb-2 font-semibold" style="color: #92400e;">⚠️ Important Notice</p>
				<p class="text-sm" style="color: #92400e;">
					Deleting this branch will:
				</p>
			...
			{#if error}
				<div class="mb-4 rounded-lg p-3 text-center text-sm" style="background-color: #fee2e2; color: #dc2626;">
					{error}
				</div>
			{/if}
