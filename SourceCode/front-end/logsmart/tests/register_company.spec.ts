...
test('register_company', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Company Address' }).fill('TestAddress1, ABC\nSecond Line,\n2!');
  ...
  await page.getByRole('textbox', { name: 'Email' }).fill('testuser2@logsmart.app');
  ...
  await expect(page.locator('span')).toContainText('testuser2@logsmart.app');
  await expect(page.locator('body')).toContainText('Test User');
  await expect(page.locator('body')).toContainText('testuser2@logsmart.app');
