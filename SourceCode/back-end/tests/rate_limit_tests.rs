...
async fn test_rate_limit_login_allows_within_limit() {
    ...
    for i in 0..5 {
        ...
        assert!(
            allowed,
            "Request {} should be allowed (limit is 5 per minute)",
            i + 1
        );
    }
}
...
async fn test_rate_limit_register_allows_within_limit() {
    ...
    for i in 0..3 {
        ...
        assert!(
            allowed,
            "Request {} should be allowed (limit is 3 per hour)",
            i + 1
        );
    }
}
...
async fn test_rate_limit_general_allows_many_requests() {
    ...
    for i in 0..60 {
        ...
        assert!(
            allowed,
            "Request {} should be allowed (limit is 60 per minute)",
            i + 1
        );
    }
}
...
async fn test_rate_limit_middleware_allows_first_request() {
    ...
    let response = ServiceExt::<Request<Body>>::oneshot(&mut service, request)
        .await
        .unwrap();
...
async fn test_rate_limit_middleware_blocks_after_limit() {
    ...
    let response = ServiceExt::<Request<Body>>::oneshot(&mut service, blocked_request)
        .await
        .unwrap();
...
async fn test_rate_limit_clone_state() {
    ...
    assert!(
        !state2.check_login(ip),
        "Cloned state should share the same limiters"
    );
}
...
async fn test_rate_limit_multiple_endpoint_types() {
    ...
    for _ in 0..60 {
        assert!(
            state.check_general(ip),
            "General should still work (different limiter)"
        );
    }
}
...
async fn test_rate_limit_concurrent_requests_same_ip() {
    ...
    for _ in 0..10 {
        ...
        set.spawn(async move { state_clone.check_login(ip) });
    }
...
async fn test_rate_limit_response_includes_retry_after() {
    ...
    for _ in 0..5 {
        ...
        let _ = ServiceExt::<Request<Body>>::oneshot(&mut service, request)
            .await
            .unwrap();
    }
...
async fn test_rate_limit_botnet_scenario() {
    ...
    for i in 0..20 {
        ...
        for _ in 0..5 {
            assert!(
                state.check_login(ip),
                "Different IPs should each get 5 attempts"
            );
        }
    }
