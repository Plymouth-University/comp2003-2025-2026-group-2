...
async fn test_get_company_clock_events_branch_filter() {
	...
	let branch_a = db::create_branch(&pool, company.id.clone(), "Branch A".to_string(), "A St".to_string())
		.await
		.expect("Failed to create branch A");

	let branch_b = db::create_branch(&pool, company.id.clone(), "Branch B".to_string(), "B St".to_string())
		.await
	...
	let events_branch_a = db::get_company_clock_events(&pool, &company.id, None, None, Some(branch_a.id.clone()))
		.await
	...
	let events_branch_b = db::get_company_clock_events(&pool, &company.id, None, None, Some(branch_b.id.clone()))
		.await
