...
mod template_service_tests {
	...
	fn test_validate_template_layout_negative_min_length() {
		...
		let (status, err) = result.unwrap_err();
		assert_eq!(status, StatusCode::BAD_REQUEST);
		assert!(err["error"].to_string().contains("min_length must be non-negative"));
	}
	...
	fn test_validate_template_layout_negative_max_length() {
		...
		let (status, err) = result.unwrap_err();
		assert_eq!(status, StatusCode::BAD_REQUEST);
		assert!(err["error"].to_string().contains("max_length must be non-negative"));
	}
	...
	fn test_validate_template_layout_min_greater_than_max() {
		...
		let (status, err) = result.unwrap_err();
		assert_eq!(status, StatusCode::BAD_REQUEST);
		assert!(err["error"].to_string().contains("min_length cannot be greater than max_length"));
	}
	...
	fn test_validate_template_layout_valid_input_types() {
		for input_type in &["text", "int", "float"] {
			...
			let layout = vec![field];
			assert!(TemplateService::validate_template_layout(&layout).is_ok(), "Failed for input_type: {}", input_type);
		}
	}
...
impl TemplateService {
	...
	fn validate_template_layout(template_layout: &logs_db::TemplateLayout) -> Result<(), (StatusCode, serde_json::Value)> {
		for (field_index, field) in template_layout.iter().enumerate() {
			...
			if let Err(e) = utils::validate_length_constraints(field.props.min_length, field.props.max_length) {
				return Err((
					StatusCode::BAD_REQUEST,
